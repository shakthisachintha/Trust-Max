...
$act=$db->insertrec("update mlm_reward set status='0' where id='$id'");

if($act)
{

header("location:reward.php?inactsucc");

echo "<script>window.location='reward.php?inactsucc';</script>";
...
$act=$db->insertrec("update mlm_reward set status='1' where id='$id'");

if($act)
{

header("location:reward.php?actsucc");

echo "<script>window.location='reward.php?actsucc';</script>";
...
$det=$db->insertrec("delete from mlm_reward where id='$id'");

if($det)
{

header("location:reward.php?del");

echo "<script>window.location='reward.php?del';</script>";
...
$sql = "delete from mlm_reward where id='$del_id'";
$result = $db->insertrec($sql);
}

if($result){?> <script>
window.location="reward.php?del";
</script> <?php
}
 }
...
$sql = "update mlm_reward set status='1' where id='$act_id'";
$result = $db->insertrec($sql);
}

if($result){?> <script>
window.location="reward.php?actsucc";
</script> <?php
}
 }
...
$sql = "update mlm_reward set status='0' where id='$inact_id'";
$result = $db->insertrec($sql);
}

if($result){?> <script>
window.location="reward.php?inactsucc";
</script> <?php
}
 }
						  ...
						  ?> 
                           ...
                           <form action="" method="post">
							<div class="row-fluid">
								...
								<span style="float:right; padding-right:5px;"><a href="addreward.php" style="color:#FFFFFF;">+ ADD Reward</a></span>
								...
								<table class="table table-striped table-bordered table-hover" id="sample-table-2">
									...
									<?php
									$plan=$db->get_all("select * from mlm_reward order by id ASC");
									$i=1;
									$num=$db->numrows("select * from mlm_reward");
									...
									?>
										<tr>
											...
											<td><?php echo $i; ?></td>
											<td><img src="../uploads/products/logo/mid/<?php echo $row_plans['reward_img']; ?>" width="50" height="50" style="vertical-align:middle;"/></td>
                                            <td><?php echo $row_plans['pair_complete']; ?></td>
											
											<td class="td-actions" align="center">
												<div class="hidden-phone visible-desktop action-buttons">
													<?php if($row_plans['status']=='0') { ?>
													
													<a class="red" href="reward.php?inact=<?php echo $row_plans['id'];?>" onclick="if(confirm('Are you sure to activate this record')) { return true; } else { return false; }">
														<i class="icon-certificate bigger-130" title="click to activate"></i>
													</a>
													
													<?php } if($row_plans['status']=='1') { ?>
												
												<a class="green" href="reward.php?act=<?php echo $row_plans['id']; ?>" onclick="if(confirm('Are you sure to deactivate this record')) { return true; } else { return false; }">
												...
												<a class="blue" href="editreward.php?memid=<?php echo $row_plans['id']; ?>">
                                                ...
                                                <?}else{?>
													<a class="grey" href="reward.php?delete=<?php echo $row_plans['id'];?>" onclick="if(confirm('Are you sure to delete this record')) { return true; } else { return false; }">
														<i class="icon-trash bigger-130" title="click to delete"></i>
													</a> 
													<?}?>
