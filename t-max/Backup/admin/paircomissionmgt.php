...
?>
<script>
window.location="paircomissionmgt.php?paysucc";
</script>
<?php
}
...
?>
<script>
window.location="paircomissionmgt.php?payrev";
</script>
<?php
}
...
?>
<script>
window.location="paircomissionmgt.php?del";
</script>
   ...
   <div class="main-content">
      <div class="breadcrumbs" id="breadcrumbs">
         <ul class="breadcrumb">
            ...
            <li class="active">Pair Comission</li>
         </ul>
         <!--.breadcrumb-->
      </div>
	...
	?>	
		<div class="date">					
			<form name="frmSearch" action="<?php $_SERVER['PHP_SELF']?>" method="POST">
				...
				<input type="submit" class="button" name="submit" style="margin-top:-12px;height:30px;" value="Search">
			</form>
		</div>
		<form action="" method="post">
                     <table class="table table-striped table-bordered table-hover" id="sample-table-2">
                        ...
                        <tbody>
                            ...
                            <tr>
                              ...
                              <td><?php echo date("d-m-Y", $poutInfo['date']); ?></td>
                              <td><?php
                              ...
                              <td>
								...
								<a class="red" href="paircomissionmgt.php?setpay=<?php echo $poutInfo['id'];?>" onclick="if(confirm('Are you sure want to Set Payment status to payed')) { return true; } else { return false; }">
								<i class="icon-certificate bigger-130" title="click to activate"></i>
								</a>
								<?php } if($poutInfo['status']=='1') { ?>
								<a class="green" href="paircomissionmgt.php?unsetpay=<?php echo $poutInfo['id']; ?>" onclick="if(confirm('Are you sure to Revert Payment Status')) { return true; } else { return false; }">
              ...
              <?php
				...
				if($uid!=""){
					$em=stripslashes($_REQUEST['user']);
					$search.="where user_id like '%$em%' and  amount!='' and bonus_type='1'";
				}
			
				if($from_date!="" && $to_date!=""){
					$FromDate= date("d-m-Y", strtotime($_REQUEST["fromdate"]));
					$ToDate= date("d-m-Y", strtotime($_REQUEST["todate"]));
					$search.=" where (DATE_FORMAT(FROM_UNIXTIME(date),'%d-%m-%Y') BETWEEN '$FromDate' AND '$ToDate') and amount!='' and bonus_type='1'";
				}
				
				if(empty($uid) && empty($_REQUEST['fromdate']) && empty($_REQUEST['todate']) ){
					$search.="where amount!='' and bonus_type='1'";
				}
				
				$header = array("user_id","from_id","amount","reason");
