...
$det=$db->insertrec("delete from mlm_reviews where id='$id'");

if($det)
{

header("location:reviews.php?del");

echo "<script>window.location='reviews.php?del';</script>";
...
$sql = "delete from mlm_reviews where id='$del_id'";
$result = $db->insertrec($sql);

}

if($result){?> <script>
window.location="reviews.php?del";
</script> <?php
}
 }
...
$act=$db->insertrec("update mlm_reviews set status='0' where id='$id'");

if($act)
{

header("location:reviews.php?actsucc");

echo "<script>window.location='reviews.php?actsucc';</script>";
...
$act=$db->insertrec("update mlm_reviews set status='1' where id='$id'");

if($act)
{

header("location:reviews.php?inactsucc");

echo "<script>window.location='reviews.php?inactsucc';</script>";
				...
				<div class="page-content">
					...
					<div class="row-fluid">
						...
						<div class="alert alert-block alert-success">
							 ...
							 <i class="icon-ok green"></i>
								<strong class="green">
									Reviews Unblocked Successfully !!!
								</strong>
								...
								<table class="table table-striped table-bordered table-hover" id="sample-table-2">
									<thead>
										<tr>
											...
											<th width="91">Name</th>
											<th width="118" >Profile ID</th>
											<th width="102" >Message</th>
											 
											 <th width="207" >Stars</th>
									...
									$fdb=$db->get_all("select * from mlm_reviews order by id desc");
									$i=1;
									$num=$db->numrows("select * from mlm_reviews");
									foreach($fdb as $row_fdb)
									
									{
						           
								    $userName = $com_obj->Extract_single("select user_fname from mlm_register where user_profileid='$row_fdb[profile_id]'");
									?>
										...
										<tr>
											...
											<td class="center">
												<label>
				<input type="checkbox" id="chkval[]" name="chkval[]" value="<?php echo $row_fdb['id']; ?>"  />
													<span class="lbl"></span>
												</label>
											</td>
                                            ...
                                            <td><?php echo $row_fdb['profile_id']; ?></td>
											
											<td><?php echo $row_fdb['message']; ?></td>
											
											<td><?php echo $row_fdb['stars']; ?></td>
											
											<td><?php echo date("d-m-Y",strtotime($row_fdb['crcdt'])); ?></td>
											
											<td><?php echo $row_fdb['ip']; ?></td>
											
											<td class="td-actions" align="center">
												<div class="hidden-phone visible-desktop action-buttons">
											
											     <?php if($row_fdb['status']=='0') { ?>
													
													<a class="red" href="reviews.php?inact=<?php echo $row_fdb['id'];?>" onclick="if(confirm('Are you sure to activate this record')) { return true; } else { return false; }">
														<i class="icon-certificate bigger-130" title="click to activate"></i>
													</a>
													
													<?php } if($row_fdb['status']=='1') { ?>
												
												<a class="green" href="reviews.php?act=<?php echo $row_fdb['id']; ?>" onclick="if(confirm('Are you sure to deactivate this record')) { return true; } else { return false; }">
												 ...
												 <?php if($demomode=='true'){?>
												   <a class="grey" onclick="demo()">
													...
													<?}else{?>
													<a class="grey" href="reviews.php?delete=<?php echo $row_fdb['id'];?>" onclick="if(confirm('Are you sure to delete this record')) { return true; } else { return false; }">
														<i class="icon-trash bigger-130" title="click to delete"></i>
													</a><?}?>
												 </div>
												</td>
								...
								<!--<div class="modal-footer">
									<button class="btn btn-small btn-danger pull-left btn-info" data-dismiss="modal">
										<i class="icon-remove"></i>
										Close
									</button>
