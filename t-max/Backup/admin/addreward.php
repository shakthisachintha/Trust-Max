...
if(isset($_REQUEST['submit']))
{
	...
	if($logo == "")
	{
		header("Location:reward.php?error");
		exit;
	} 
	else 
	{
	...
	$img_size = filesize($_FILES['logo']['tmp_name']);
			...
			{
				header("Location:reward.php?largeimage");
				exit;
			}
			else
			{
		...
		{
			header("Location:reward.php?not-a-image");
			exit;
		}
	}
}
	
	if(($pair_from == "") or ($logo == "")){
		header("Location:reward.php?error");
		echo "<script>location.href='reward.php?error'</script>";
		exit;
	}
		
	
 $insert = $db->insertrec("INSERT INTO mlm_reward (pair_complete,reward_img) VALUES ('$pair_from','$cate_img_small')");

	if($insert)
	 { 
	 header("Location:reward.php?succ"); 
	 ?>
	<script>
	window.location="reward.php?succ";
	</script>	
	<?php
	}
}
			...
			<div class="main-content">
				<div class="breadcrumbs" id="breadcrumbs">
					<ul class="breadcrumb">
						...
						<li>
							<a href="reward.php">Reward Plan</a>
