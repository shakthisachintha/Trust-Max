...
$act=$db->insertrec("update mlm_level_commission set status='0' where id='$id'");

if($act)
{
header("location:levelcommission.php?inactsucc");
echo "<script>window.location='levelcommission.php?inactsucc';</script>";
}
}
...
$act=$db->insertrec("update mlm_level_commission set status='1' where id='$id'");

if($act)
{

header("location:levelcommission.php?actsucc");

echo "<script>window.location='levelcommission.php?actsucc';</script>";
...
$det=$db->insertrec("delete from mlm_level_commission where id='$id'");

if($det)
{

header("location:levelcommission.php?del");

echo "<script>window.location='levelcommission.php?del';</script>";
...
$sql = "delete from mlm_level_commission where id='$del_id'";
$result = $db->insertrec($sql);
}

if($result){?> <script>
window.location="levelcommission.php?del";
</script> <?php
}
 }
...
$sql = "update mlm_level_commission set status='1' where id='$act_id'";
$result = mysql_query($sql);
}

if($result){?> <script>
window.location="levelcommission.php?actsucc";
</script> <?php
}
 }
...
$sql = "update mlm_level_commission set status='0' where id='$inact_id'";
$result = mysql_query($sql);
}

if($result){?> <script>
window.location="levelcommission.php?inactsucc";
</script> <?php
}
 }
			...
			<div class="main-content">
				<div class="breadcrumbs" id="breadcrumbs">
					<ul class="breadcrumb">
						...
						<li class="active">Level Commission Settings </li>
					</ul><!--.breadcrumb-->
						  ...
						  ?> 
						   ...
						   <div class="alert alert-block alert-error">
							 ...
							 <i class="icon-off red"></i>
								<strong class="red">
									Level settings blocked Successfully !!!
								</strong>
                           ...
                           <form action="" method="post">
							<div class="row-fluid">
								...
								Level Commission Settings Management
								
								<span style="float:right; padding-right:5px;"><a href="addlevel.php" style="color:#FFFFFF;">+ ADD Level</a></span>
								...
								<table class="table table-striped table-bordered table-hover" id="sample-table-2">
									...
									<?php
									$plan=$db->get_all("select * from mlm_level_commission order by id ASC");
									$i=1;
									$num=$db->numrows("select * from mlm_level_commission");
									...
									?>
										<tr>
											...
											<td><?php echo $i; ?></td>
											<td><?php echo $row_plans['level']; ?></td>
                                            <td><?php echo $row_plans['percentage']; ?></td>
											<td class="td-actions" align="center">
												<div class="hidden-phone visible-desktop action-buttons">
													<?php if($row_plans['status']=='0') { ?>
													
													<a class="red" href="levelcommission.php?inact=<?php echo $row_plans['id'];?>" onclick="if(confirm('Are you sure to activate this record')) { return true; } else { return false; }">
														<i class="icon-certificate bigger-130" title="click to activate"></i>
													</a>
													
													<?php } if($row_plans['status']=='1') { ?>
												
												<a class="green" href="levelcommission.php?act=<?php echo $row_plans['id']; ?>" onclick="if(confirm('Are you sure to deactivate this record')) { return true; } else { return false; }">
												...
												<a class="blue" href="editlevel.php?memid=<?php echo $row_plans['id']; ?>">
                                      ...
                                      <?php if($demomode=='true'){?>
									   <a class="grey" onclick="demo()">
										...
										<?}else{?>
													<a class="grey" href="levelcommission.php?delete=<?php echo $row_plans['id'];?>" onclick="if(confirm('Are you sure to delete this record')) { return true; } else { return false; }">
														<i class="icon-trash bigger-130" title="click to delete"></i>
													</a> <?}?>
												  </div>
												</td>
