...
{
	$plan_name = addslashes($_REQUEST['mem_name']);
	$amount=addslashes($_REQUEST['act_amt']);
	$days=addslashes($_REQUEST['days']);
	$capping_amt=addslashes($_REQUEST['capping_amt']);
	
	if(($plan_name == "") or ($amount == "") or ($days == ""))
	{
		header("Location:membership.php?error");
		exit;
	}
		
 $insert = $db->insertrec("INSERT INTO mlm_membership (membership_name,act_amount,days,capping_amt) VALUES ('$plan_name','$amount','$days','$capping_amt')");
						...
						<div class="span12">
							...
							<form class="form-horizontal" name="general"  method="post" action="" onsubmit="return planvalidate();" enctype="multipart/form-data" />
								<div class="control-group">
	                               ...
	                               <div class="control-group">
									<label class="control-label" for="form-field-2">Amount (<?php echo $sitecurrency; ?>)<span style="color:#FF0000;">*</span> : </label>
								...
								<div class="control-group">
									...
									<div class="controls">
										<input type="text" name="capping_amt" id="capping_amt_id" />
									</div>
								</div>
