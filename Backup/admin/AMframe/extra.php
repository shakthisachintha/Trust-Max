...
class extra extends common {
	...
	public function tooltip($profileid) {
		...
		$refCount=database::singlerec("select count(*) as tot from mlm_register where user_placementid='$profileid'");
		...
		$disp.="<tr><td align='left'>Name :</td><td align='left'>$name</td></tr>";
		$disp.="<tr><td align='left'>Profile Id :</td><td align='left'>$profileid</td></tr>";
		$disp.="<tr><td align='left'>Sponser Id :</td><td align='left'>$uInfo[user_sponserid]</td></tr>";
		$disp.="<tr><td align='left'>Sponser Name :</td><td align='left'>$spInfo[user_fname] $spInfo[user_lname]</td></tr>";
		$disp.="<tr><td align='left'>Refered :</td><td align='left'>$refCount[tot] user(s)</td></tr>";
	...
	public function usertree($profileid,$spnsr,$count) {
		...
		for($i=0;$i<$dispcount;$i++) {
			if(count($profileid)<2) $profileid[]=0;
			$user_profileid=$profileid[$i];
			$userInfo=database::singlerec("select * from mlm_register where user_profileid='$user_profileid'");
			if($i%2==0) {
				if(!empty($userInfo)) {
					echo "<li>";
					echo '<a href="binary.php?uid='.$user_profileid.'" class="tooltip1"><img src="images/no-blue.jpg">';
					...
					echo '<li><a style="cursor:default;"><img src="images/no-blue.jpg">';
