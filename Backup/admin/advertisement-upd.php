...
if(!isset($_REQUEST['edit']) || empty($_REQUEST['edit'])) {
	header("location:advertisement-list.php");
	echo "<script>window.location='advertisement-list.php';</script>";
	exit;
}

$edit_id = addslashes($_REQUEST['edit']);
$menu7='class="active"';

if(isset($upd_frmsubmit)) {
	$cur_date = date("Y-m-d");
	$idv = addslashes($idv);
	...
	if(isset($_FILES['img']['tmp_name']) && !empty($_FILES['img']['tmp_name'])) {
		$adv_image = $_FILES['img']['name'];
		$img_size = filesize($_FILES['img']['tmp_name']);		
		if($img_size > 1048576) 
		{
			echo "<script>location.href='advertisement-upd.php?edit=$idv&largeimage'</script>";
			header("Location:advertisement-upd.php?edit=$idv&largeimage");
			exit;
		}
		else
		{
			...
			{		
				...
				$getimg = $db->singlerec("select ad_img from mlm_advertise where id='$idv'");
				$img = $getimg['ad_img'];
			...
			{
				echo "<script>location.href='advertisement-upd.php?edit=$idv&not-a-image'</script>";
				header("Location:advertisement-upd.php?edit=$idv&not-a-image");
				exit;
			}
		}
	}
	
	$db->insertrec("update mlm_advertise set $set where id='$idv'");
	
	header("location:advertisement-list.php?updsuc");
	echo "<script>window.location='advertisement-list.php?updsuc';</script>";
	exit;
}

$getdet = $db->singlerec("select * from mlm_advertise where id='$edit_id'");
$img = $getdet['ad_img'];
if(!empty($img) && file_exists("../uploads/advertisement/$img")) {
	$img_src = "<img src='../uploads/advertisement/$img' />";
}
else {
	$img_src = "";
}
						...
						<div class="span12">
						 ...
						 <?php if(isset($_REQUEST['largeimage'])) { ?> 					  
						   ...
						   <?php } ?>
							...
							<input type="hidden" name="idv" value="<?php echo $edit_id;?>" />
								<div class="control-group">
									...
									<div class="controls">
										...
										<?php foreach($GT_ad_pg as $ad_pg) {?>
											<option value="<?=$ad_pg;?>" <?php if($getdet['ad_location'] == $ad_pg){ echo "selected"; }?>><?=$ad_pg;?></option>
										<?php } ?>
										</select>
									</div>
								</div>
								<div class="control-group">
									...
									<div class="controls">
										<input type="text" name="dimension" id="dimId" value="<?=$getdet['ad_dimension'];?>" readonly />
									</div>
								</div>
								<div class="control-group">
									<label class="control-label" for="form-field-1">Link <span style="color:#FF0000;">*</span> : </label>
									<div class="controls">
										<input type="text" name="link" id="linkId" value="<?=$getdet['ad_link'];?>" />
									</div>
								</div>
								<div class="control-group">
									<label class="control-label" for="form-field-1">Image <span style="color:#FF0000;">*</span> : </label>
									<div class="controls">
										<input type="file" name="img" id="imageId" />
									</div>
									<?php echo $img_src;?>
								</div>
								<p><b>NOTE : Accepted file format png, jpg, jpeg, gif with maximum size of 1 MB. </b></p>
								<div class="form-actions">
									<input type="submit" name="upd_frmsubmit" value="SUBMIT" class="btn btn-info" style="font-weight:bold;" />&nbsp; &nbsp; &nbsp;
