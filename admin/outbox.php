...
if($del)
{
header("location:outbox.php?del");
echo "<script>window.location='outbox.php?del';</script>";
}
...
if($result){?> <script>
window.location="outbox.php?del";
</script> <?php
}
 }
		...
		<div class="main-container container-fluid">
			...
			<div class="main-content">
                          ...
                          <form action="" method="post">
							...
							<div class="row-fluid">
								...
								<table class="table table-striped table-bordered table-hover" id="sample-table-2">
									...
									$inbdb=$db->get_all("select * from mlm_outbox where (outbox_userid='admin' and outbox_fromstatus='0') order by outbox_id desc");
									$i=1;
									$num=$db->numrows("select * from mlm_outbox where (outbox_userid='admin' and outbox_fromstatus='0') order by outbox_id desc");
									
									foreach($inbdb as $row_inbdb)
									{
									if($row_inbdb['outbox_usertype'] == '2')
									{	
									$useinb=$db->singlerec("select * from mlm_register where user_id='$row_inbdb[outbox_toupid]'");
				...
				<input type="checkbox" id="chkval[]" name="chkval[]" value="<?php echo $row_inbdb['outbox_id']; ?>"  />
											...
											<td>
											<?php if($row_inbdb['outbox_readstatus']=="1")
											 {
											echo "Read";
											 } else 
											 { 
											...
											<a href="fwdmail.php?fwd=<?php echo $row_inbdb['outbox_id']; ?>" >
													<img src="images/forward.png" alt="reply" title="click to forward" style="vertical-align:top; margin-top:2px; width:18px; height:18px;"/></a>
