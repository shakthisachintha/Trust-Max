...
$del=$db->insertrec("delete from mlm_outbox where outbox_id='$delid'");
if($del)
{
header("location:inbox.php?del");
echo "<script>window.location='inbox.php?del';</script>";
}
...
{
$del_id = $checkbox[$i];
$del=$db->insertrec("delete from mlm_outbox where outbox_id='$del_id'");
}}
if($del)
{
header("location:inbox.php?del");
		...
		<div class="main-container container-fluid">
			...
			<div class="main-content">
								...
								<table class="table table-striped table-bordered table-hover" id="sample-table-2">
									<thead>
										<tr><th width="39">&nbsp;</th>
											...
											<th width="66">Sl.No</th>
											
										    <th width="125">Sender Name</th>
											
										     <th width="152" >Profile Id</th>
											 
											 <th width="179" >Subject</th>
											 
											 <th width="171" >Date</th>
									...
									<?php 
									$inbdb=$db->get_all("select * from mlm_outbox where outbox_toprofileid='Admin' order by outbox_id desc");
									$i=1;
									
									$num=$db->numrows("select * from mlm_outbox where outbox_toprofileid='Admin' order by outbox_id desc");
									
									foreach($inbdb as $row_inbdb)
									{
									if($row_inbdb['outbox_userid'] != 'admin')	
									{	
									$useinb=$db->singlerec("select * from mlm_register where user_id='$row_inbdb[outbox_userid]'");
									...
									?>
										...
										<tr <?php if($row_inbdb['outbox_readstatus']=="0") { ?> style="font-weight:bold;" <?php }  else { ?> style="font-weight:normal;" <?php } ?>>
									
								<td><?php if($row_inbdb['outbox_readstatus']=="0") { ?> <img src="images/unreadmsg.jpeg" style="width:24px; height:24px;"> <?php }  else { ?> <img src="images/readmsgg.jpeg" style="width:20px; height:20px;"> <?php } ?>
										</td>
				...
				<input type="checkbox" id="chkval[]" name="chkval[]" value="<?php echo $row_inbdb['outbox_id']; ?>"  />
                                            ...
                                            <td><?php echo $row_inbdb['outbox_profileid']; ?></td>
											...
											<td><?php echo date("d-m-Y h:i:s",strtotime($row_inbdb['outbox_date'])); ?></td>
												...
												<a href="view_mail.php?detail=<?php echo $row_inbdb['outbox_id'];?>&usrviewed" >
													<img src="images/view_icon.gif" alt="view" title="click to view" style="vertical-align:top; margin-top:2px;"/>
													
													</a>
												<a href="replymail.php?rep=<?php echo $row_inbdb['outbox_id'];?>" >
													<img src="images/reply-icon.png" alt="reply" title="click to reply" style="vertical-align:top; margin-top:2px; width:18px; height:18px;"/>
													
													</a>
													
														<a href="fwdmail.php?fwd=<?php echo $row_inbdb['outbox_id'];?>" >
													<img src="images/forward.png" alt="reply" title="click to forward" style="vertical-align:top; margin-top:2px; width:18px; height:18px;"/>
